...
# МОДУЛЬ 5: ESKD_2.124_MODULE (СОГЛАСОВАНИЕ И ИМПОРТНЫЕ ДАННЫЕ)
## Файл: `eskd_2.124_module.xsd` | Версия: 1.0 | Namespace: `http://gost.ru/eskd/2.124/2024`

### Раздел 1: Маршрут согласования (ApprovalRoute)

| Сущность | Элемент/Атрибут | Тип | Обяз. | Толкование | ГОСТ |
|----------|-----------------|-----|-------|------------|------|
| **ApprovalRouteType** | — | ComplexType | — | Тип для описания этапа согласования позиции перечня | ГОСТ 2.124-2014, п. 4.2 |
| | `stage` | ApprovalStageEnum | **required** | Этап согласования: `draft`, `under_review`, `approved`, `rejected`, `voided` | ГОСТ 2.124-2014, п. 4.2 |
| | `date` | DateType | **required** | Дата перехода в текущий этап (ГГГГ-ММ-ДД) | ГОСТ 2.304 |
| | `responsiblePerson` | BasePersonType или ShortText | **required** | Ответственное лицо (ФИО или ссылка на person из 2.058) | ГОСТ 2.124-2014, п. 4.2 |
| | `comment` | LongText | optional | Комментарий согласующего, причина решения | ГОСТ 2.124-2014, п. 4.2 |

**Примечание:** Элемент `approvalHistory` типа `ApprovalRouteType` может повторяться多次 (`maxOccurs="unbounded"`) в составе `AllowedComponentItemType` для отражения полной истории согласования.

### Раздел 2: Данные об импортном компоненте (ImportedComponentData)

| Сущность | Элемент/Атрибут | Тип | Обяз. | Толкование | ГОСТ |
|----------|-----------------|-----|-------|------------|------|
| **ImportedComponentDataType** | — | ComplexType | — | Тип для описания данных об импортном компоненте | ГОСТ 2.124-2014, п. 5.1 |
| | `certificateNumber` | CodeString | **required** | Номер сертификата соответствия или разрешения на применение | ГОСТ 2.124-2014, п. 5.1 |
| | `certificateDate` | DateType | **required** | Дата выдачи сертификата/разрешения | ГОСТ 2.304 |
| | `issuingAuthority` | BaseOrganizationType или ShortText | **required** | Орган, выдавший сертификат (наименование или ссылка на organization) | ГОСТ 2.124-2014, п. 5.1 |
| | `technicalDocumentation` | xs:anyURI | optional | Ссылка на техническую документацию зарубежного изготовителя | ГОСТ 2.124-2014, п. 5.1 |

**Примечание:** Элемент `importData` типа `ImportedComponentDataType` является опциональным (`minOccurs="0"`) в составе `AllowedComponentItemType`.

---

# МОДУЛЬ 6: ESKD_COMPLIANCE_CHECK (ПРОВЕРКА СООТВЕТСТВИЯ)
## Файл: `eskd_compliance_check.xsd` | Версия: 1.0 | Namespace: `http://gost.ru/eskd/compliance/2024`

### Раздел 1: Статусы проверки

| Сущность | Значение | Толкование | ГОСТ |
|----------|----------|------------|------|
| **CheckStatusEnum** | `passed` | Соответствует — компонент найден в перечне, все параметры совпадают | ГОСТ 2.111-2013, п. 4.2.5 |
| | `failed` | Не соответствует — компонент отсутствует в перечне или запрещён | ГОСТ 2.111-2013, п. 4.2.5 |
| | `warning` | Требует проверки — компонент имеет ограничения или устарел | ГОСТ 2.111-2013, п. 4.2.5 |
| **OverallComplianceStatusEnum** | `full_compliance` | Полное соответствие — все позиции имеют статус `passed` | ГОСТ 2.053-2023, п. 7.2.1 |
| | `partial_compliance` | Частичное соответствие — имеются `warning` или единичные `failed` | ГОСТ 2.053-2023, п. 7.2.1 |
| | `non_compliant` | Не соответствует — критическое количество `failed` | ГОСТ 2.053-2023, п. 7.2.1 |
| | `not_verified` | Не проверено — проверка не выполнена | ГОСТ 2.053-2023, п. 7.2.1 |

### Раздел 2: Проверка отдельного компонента (ComplianceCheckType)

| Сущность | Элемент/Атрибут | Тип | Обяз. | Толкование | ГОСТ |
|----------|-----------------|-----|-------|------------|------|
| **ComplianceCheckType** | — | ComplexType | — | Тип для описания результата проверки одного компонента | ГОСТ 2.111-2013, п. 6.3 |
| | `componentRef` | xs:IDREF | **required** | Ссылка на компонент в спецификации изделия (eskd_product или ekb_component) | ГОСТ 2.053-2023, п. 5.2.4 |
| | `allowedItemRef` | xs:IDREF | optional | Ссылка на позицию в разрешённом перечне (allowed_component_item) | ГОСТ 2.124-2014, п. 4.3 |
| | `status` | CheckStatusEnum | **required** | Результат проверки: `passed`, `failed`, `warning` | ГОСТ 2.111-2013, п. 5.1.2 |
| | `comment` | LongText | optional | Пояснение результата, причина несоответствия, рекомендации | ГОСТ 2.111-2013, п. 6.3 |
| | `normControlRef` | xs:IDREF | optional | Ссылка на нормоконтролёра (person из 2.058), выполнившего проверку | ГОСТ 2.111-2013, п. 5.1.2 |

### Раздел 3: Отчёт о проверке (ComplianceReportType)

| Сущность | Элемент/Атрибут | Тип | Обяз. | Толкование | ГОСТ |
|----------|-----------------|-----|-------|------------|------|
| **ComplianceReportType** | — | ComplexType | — | Тип отчёта о проверке соответствия всего изделия | ГОСТ 2.111-2013, п. 6.3 |
| | Наследует | BaseEntityType | — | Базовые метаданные (id, description, changeAttGroup и др.) | ГОСТ Р 2.058-2023 |
| | `reportId` | CodeString | **required** | Уникальный идентификатор отчёта (например, `НК-2024-001`) | ГОСТ 2.111-2013, п. 6.3 |
| | `productRef` | xs:IDREF | **required** | Ссылка на проверяемое изделие (eskd_product) или ЭСК | ГОСТ 2.053-2023, п. 5.1 |
| | `allowedListRef` | xs:IDREF | **required** | Ссылка на разрешённый перечень (allowed_component_list) | ГОСТ 2.124-2014, п. 4.3 |
| | `verificationDate` | DateType | **required** | Дата выполнения проверки | ГОСТ 2.111-2013, п. 6.3 |
| | `overallStatus` | OverallComplianceStatusEnum | **required** | Итоговый статус проверки всего изделия | ГОСТ 2.053-2023, п. 7.2.1 |
| | `checks` | Sequence of ComplianceCheckType | **required**, min 1 | Последовательность проверок отдельных компонентов (минимум 1) | ГОСТ 2.053-2023, п. 5.2.4 |
| | `verifier` | xs:IDREF | optional | Ссылка на организацию или лицо, выполнившее проверку | ГОСТ 2.111-2013, п. 5.1.2 |
| | `remarks` | LongText | optional | Общие замечания по результатам проверки, рекомендации | ГОСТ 2.111-2013, п. 6.3 |

**Корневой элемент:** `compliance_report` типа `ComplianceReportType` предназначен для обмена отчётами между системами.

---

# МОДУЛЬ 7: ESKD_DOCUMENT_CLASSIFICATION (КЛАССИФИКАЦИЯ ДОКУМЕНТОВ)
## Файл: `eskd_document_classification.xsd` | Версия: 1.0 | Namespace: `http://gost.ru/eskd/docclass/2024`

### Раздел 1: Коды видов документов (DocumentKindCodeEnum)

| Код | Наименование | Описание | ГОСТ |
|-----|--------------|----------|------|
| `СП` | Спецификация | Текстовый КД, определяющий состав сборочной единицы | ГОСТ 2.102-2023, п. 5.3.1 |
| `ЭСК` | Электронная конструктивная структура изделия | Электронный КД, определяющий состав изделия | ГОСТ 2.053-2023, п. 5.1; ГОСТ 2.102-2023, п. 5.2.1.1 |
| `ВП` | Ведомость покупных изделий | Перечень покупных изделий, применяемых в изделии | ГОСТ 2.102-2023, п. 5.3.4 |
| `ВР` | Ведомость разрешения применения покупных изделий | Документ, утверждающий разрешённые покупные изделия | ГОСТ 2.124-2014, п. 4.3; ГОСТ 2.102-2023, п. 5.3.5 |
...
| `СБ` | Сборочный чертеж | Чертёж, показывающий соединение частей изделия | ГОСТ 2.102-2023, п. 5.3.3 |
| `ЧД` | Чертеж детали | Чертёж отдельной детали | ГОСТ 2.102-2023, п. 5.3.3 |
| `ПМ` | Программа и методика испытаний | Документ, устанавливающий порядок испытаний | ГОСТ 2.102-2023, п. 5.3.9 |
| `Д` | Документ (общий случай) | Общий тип для иных видов документов | ГОСТ 2.102-2023, п. 5.3.10 |

**Примечание:** Полный перечень доступен в типе `DocumentKindCodeEnum` модуля `eskd_document_classification.xsd`.

### Раздел 2: Дополнительные поля в AllowedComponentListType и AllowedComponentItemType

| Сущность | Элемент/Атрибут | Тип | Обяз. | Толкование | ГОСТ |
|----------|-----------------|-----|-------|------------|------|
| **AllowedComponentListType** (дополнено) | `purchasedItemsListRef` | xs:IDREF | optional | Ссылка на документ типа **ВП** (Ведомость покупных изделий) | ГОСТ 2.102-2023, п. 5.3.4 |
| | `permissionListRef` | xs:IDREF | optional | Ссылка на документ типа **ВР** (Ведомость разрешения применения) | ГОСТ 2.124-2014, п. 4.3 |
| **AllowedComponentItemType** (дополнено) | `permissionDocumentRef` | xs:IDREF | optional | Ссылка на позицию в документе типа **ВР** для конкретной позиции | ГОСТ 2.124-2014, п. 4.3 |
| | `importData` | ImportedComponentDataType | optional | Данные об импортном компоненте (из модуля 2.124) | ГОСТ 2.124-2014, п. 5.1 |
| | `approvalHistory` | ApprovalRouteType | optional, unbounded | История согласования позиции (из модуля 2.124) | ГОСТ 2.124-2014, п. 4.2 |

### Раздел 3: Поддержка ЭСК как альтернативы спецификации

| Сущность | Тип | Описание | ГОСТ |
|----------|-----|----------|------|
| **StructureDocumentType** | Abstract ComplexType | Абстрактный тип для документов структуры изделия | ГОСТ 2.102-2023, п. 5.2.1.1 |
| **Specification** | Подтип StructureDocumentType | Для документов типа `СП` (Спецификация) | ГОСТ 2.102-2023, п. 5.3.1 |
| **ElectronicStructure** | Подтип StructureDocumentType | Для документов типа `ЭСК` (Электронная конструктивная структура) | ГОСТ 2.053-2023, п. 5.1 |

**Примечание:** В элементах `productRef` (в `ComplianceReportType` и других типах) допускается ссылка как на `eskd_product`, так и на объект электронной конструктивной структуры.
